...
To find the source of the hump, we break out each order into it's own graph. From this graph, we can see that the hump is cause by order Anseriformes (ducks). The order that makes up the most of the data set appears to be order Passeriformes (songbirds). In addition, this order appears to have a normal distribution. From this point on, our research will focus on this order to reduce outliers and improve distribution.
...
Here we are looking for any trends in the length mass ratio of Passeriformes and we find there is a slight upward trend in the regression line that becomes more pronounced as altitude increases.
